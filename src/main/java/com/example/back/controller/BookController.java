...
public class BookController {
    ...
    @GetMapping("/list")
    public Result listBooks(@RequestParam Integer page, @RequestParam Integer size) {
        List<Book> books = bookService.listBooks(page, size);
        for (Book book : books) {
    ...
    public Result getBooksByType(@PathVariable String book_type,
        ...
        List<Book> books = bookService.getBooksByType(book_type, page, size);
        for (Book book : books) {
            if (book.getBook_img() != null) {
                String book_img_base64 = Base64.getEncoder().encodeToString(book.getBook_img());
                book.setBook_img_base64(book_img_base64);
    ...
    public Result getBooksByTitleContaining(@RequestParam String title,
                                            @RequestParam Integer page,
                                            @RequestParam Integer size) {
        List<Book> books = bookService.getBooksByTitleContaining(title, page, size);
        return Result.success(books);
    }
    ...
    public Result getUnreviewedBooks(@RequestParam Integer page, @RequestParam Integer size) {
        List<Book> books = bookService.getUnreviewedBooks(page, size);
        return Result.success(books);
    }
    ...
    @GetMapping("/banned")
    public Result getBannedBooks(@RequestParam Integer page, @RequestParam Integer size) {
        List<Book> books = bookService.getBannedBooks(page, size);
        return Result.success(books);
    }
    ...
    public Result getBooksBySellerId(@PathVariable String sellerId,
                                     @RequestParam Integer page,
                                     @RequestParam Integer size) {
        List<Book> books = bookService.getBooksBySellerId(sellerId, page, size);
